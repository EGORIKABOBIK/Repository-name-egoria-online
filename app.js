...
function setScreen(name) {
  ["setupScreen", "authScreen", "appScreen"].forEach((id) =>
...
function getConnection() {
  return {
    url:
      localStorage.getItem("egoria_supabase_url") ||
      DEFAULT_SUPABASE_URL,
    key:
      localStorage.getItem("egoria_supabase_key") ||
  ...
  try {
    if (state.authMode === "login") {
      ...
      if (!/^[a-zA-Z0-9_]{3,24}$/.test(username)) {
        throw new Error(
          "Юз: 3–24 символа, только буквы, цифры и _."
        );
      }
      ...
      if (existing) {
        throw new Error("Этот юз уже занят.");
      }
      ...
      if (!data.session) {
        showToast(
          "Аккаунт создан. Подтверди email, если Supabase попросил.",
          "success"
        );
      }
    }
    ...
    if (message.toLowerCase().includes("database error")) {
      showToast(
        "Не удалось создать аккаунт. Возможно, юз уже занят.",
        "error"
      );
    } else {
...
function daysWithoutMessages(value) {
  if (!value) return 0;
    ...
    try {
      ...
      if (avatar) {
        if (
          ...
          throw new Error(
            "Аватар должен быть меньше 5 МБ."
          );
        }
...
function setupInstallExperience() {
  ...
  const closeHint = () => {
    ...
    sessionStorage.setItem(
      "egoria_install_hint_closed",
      "1"
    );
  };

  hint.querySelector("b").onclick =
    (event) => {
  ...
  hint.onclick = async () => {
    ...
    if (isIOS) {
      alert(
        'На iPhone нажми кнопку «Поделиться» в Safari, затем выбери «На экран Домой».'
      );
    } else {
      alert(
